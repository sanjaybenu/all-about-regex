...
Regex is a powerful tool. As specified above, one of the usage of this tool is to validate data. Regex is used to match the data with a predefined sequence of characters. Some common examples of data validation are:
...
In this tutorial we will look at the following Regex pattern. 
```
/^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z]{2,6})$/
```
This Regex pattern defines a valid email address.
We will be using this Regex pattern to understand some of the different components of the Regex. Since there are more components than used in this example, they have been explained with their respective examples.

In the end we will look at how this Regex pattern will validate an email address.
...
Quantifiers in regex are special characters or sequences that, ass the name suggests quantifies the number of occurrences f the preceding element. They are used to define how may times a character or combination of characters can appear. Some common quantifiers include:

- ``*`` asterisk: Matches zero or more occurrences of the preceding element.
- ``+`` plus: Matches one or more occurrences of the preceding element.
- ``?`` question mark: Matches zero or one occurrence of the preceding element.
- ``{n}`` n defines the no. of accurances of character or character group: Matches exactly `n` occurrences of the preceding element.
...
In first character class ``[a-z0-9_\.-]+`` lowercase alphabets,digits, underscore, dots and dashes can have one or more occurances. Please note this part of email will be invalid if an uppercase or special character like * or # or $ or any other special character is provided.

In the second character class ``[\da-z\.-]+`` lowercase alphabets,digits,dots and dashes can have one or more occurances.

Can you guess what does this character class ``([a-z]{2,6})`` mean?

You may have guessed it by now that in this group
lowercase aplhabets can have between 2-6 accurances.

If this is re written as ``([a-z]{2,})`` or ``([a-z]{2})``. The lowecase alphabets accurances can 2 or more in the first case and exactly 2 in the second case.
...
In the emai example there character classes are used at a few places
...
``[a-z]`` . In this instance a character in this position can match a lowercase alphabets
...
``m``- multiline: Changes the behavior of the "^" and "$" anchors to match the beginning and end of each line within a multiline string, rather than just the start and end of the entire string.
...
``([a-z0-9_\.-]+)`` allows for a pattern to define first part of email that can contain one or many occurance of lowercase alphabets,digits, underscore, dots and dashes.

``([\da-z\.-]+)`` allows for a pattern to define second part of email that can contain one or many occurance of lowercase alphabets,digits, dots and dashes.

``([a-z]{2,6})`` allows for a pattern to define third part of email that can contain 2-6 occurance of lowercase alphabets.
...
Our example of regex for email doesnot have any explicit greedy or lazy matches. Quantifiers ``+`` and ``{2-6}`` work in their own way. ``+`` matching as many occurances of of character classes whereas ``{2,6}`` limits the match to betweeen 2-6 occurances.
...
Anchors are also a type of bondary ``^`` caret symbol defines the beginning of a string and ``$`` dollar symbol defines end of the string.

### Back-references

Back-references in Regex are a feature that allows allows to refer back to a previously matched group in a pattern.
...
This will select the words that have same alphabet repeated multiple times in a word eg 'root', 'fantastic' etc.
...
Replacing ``=`` in pattern with ``!`` we can change the behaviour to not match. This is also refered to as Negative Lookahead or Negative Lookbehind.

## Example Explained

Lets look at the Original Example
...
``([a-z0-9_\.-]+)`` represents a group represented by character class representing lowercase alphabets,digits, underscore, dots and dashes and ``+`` sign represents one or more occurances of these characters. This represents the username portion of email.

``@`` is literal '@' and this means username is followed by character '@'.

``([\da-z\.-]+)\`` This group represents the domain name and can have one or more occurances of alphabets,digits, dots and dashes.

``.`` is literal '.' and this means domain name is followed by character '.'

``([a-z]{2,6})`` is the domain extension and have between 2-6 occurances of lowercase alphabets.
...
## Author
My Name is Sanjay Chopra. I am learning to code and currently going through  FullStack Developer Bootcamp through Monash University. I currently run a succesful Retail Business and hoping to be a fullstack developer.
